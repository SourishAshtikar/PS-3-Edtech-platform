...
async function main() {
  ...
  // 1. Seed Default Subject
  const hciSubject = await prisma.subject.upsert({
    where: { id: 'subj-hci' },
    update: {},
    create: {
      id: 'subj-hci',
      name: 'Human Computer Interaction',
      description: 'Master the concepts of User Interface Design and HCI.',
    }
  });
  ...
  for (const m of modulesToSeed) {
    await prisma.module.upsert({
      ...
      update: {
        ...
        subjectId: hciSubject.id
      },
      create: { ...m, subjectId: hciSubject.id }
    });
  }
